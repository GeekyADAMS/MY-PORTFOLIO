<template>
<div class="w100 flex-col portfolio main_main" id="portfolio">
  ...
  <div class="w100 web">
    <span class="ml-auto montserrat --light-purple fade-1 no-select">WEB</span>
  </div>
  ...
  <div class="flex-col w100 mt-n3 activities-container border-box s-mt-5">
    <div class="other-projects ml-auto flex-col">
      ...
      <div class="flex-row s-ml-3">
        ...
        <div class="g-line-p5"></div>
        <p class="light-purple satisfy mr-1 ml-1 w-fit">Community works</p>
      </div>
    </div>

    <div class="flex-row space-btw wrap flex-col-m w100p m-100 card-sect section" id="smooth-scroll-wrapper">

      <div class="b-card w30p m-w80 hide-overflow mb-2 mouse-focus flex-col a-c-n" v-for="(activity, index) in activities" :key="index" @mouseover="showTrace(index)" @mouseout="hideTrace(index)">
    ...
    return {
      activities: [{
        ...
        },
        {
          ...
          desc: 'Sometimes, you might want to make app fuctionality decisions based on user device, for example, say: trigger requests, enable PWA Install, disable features, give gifts, make analysis e.t.c. I explained how these can be achieved in Vue using mobile responsiveness as a case study',
          link: 'https://code-with-geeky.hashnode.dev/handling-mobile-responsiveness-and-more-like-a-boss-in-vuejs-with-vuex-ck88zmeh200429as10nsv7o45',
          tags: ['Vue', 'Vuex', 'Hashnode']
        }
      ]
    }
  },
...
.bg-lines {
  ...
  width: 100%;
  border-left: 1px dashed rgba(253, 201, 186, .2);
  border-right: 1px dashed rgba(253, 201, 186, .2);
  display: -ms-flexbox;
  display: flex;
  z-index: -1;
}
...
.lines__line+.lines__line {
  ...
  border-left-color: rgba(253, 201, 186, .2);
}
...
.project-desc {
  ...
  background: linear-gradient(to left top, rgba(255, 255, 255, .25) 0%, rgba(255, 255, 255, 0.35) 60%);
